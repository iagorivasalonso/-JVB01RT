...
public class EventoServicio {
    ...
    public List<Evento> actualizarEvento(Long id, Evento eventoModificar) {
        System.err.println(eventoModificar);
       eventoModificar.setId(id);

       return this.eventos.stream()
               .map( evento -> evento.getId().equals(id) ? eventoModificar :evento).toList();
    }

    public List<Evento> modificarEvento(Long id, Evento eventoModificar) {

        eventoModificar.setId(id);

        Evento evento = this.eventos.stream()
                .filter(evento1 -> evento1.getId().equals(id))
                .findFirst().orElse(null);

        if(eventoModificar.getNombre() != null)
        {
            evento.setNombre(eventoModificar.getNombre());
        }
        if(eventoModificar.getDescripcion() != null)
        {
            evento.setDescripcion(eventoModificar.getDescripcion());
        }
        if(eventoModificar.getFecha() != null)
        {
            evento.setFecha(eventoModificar.getFecha());
        }
        if(eventoModificar.getHora() != null)
        {
            evento.setHora(eventoModificar.getHora());
        }

        return this.actualizarEvento(id,evento);
    }

    public ResponseEntity<?> eliminarEvento(Long id) {

        Boolean estaEliminado = this.eventos.removeIf(evento -> evento.getId().equals(id));

        if(estaEliminado)
        {
            return ResponseEntity.status(HttpStatus.ACCEPTED)
                    .body(new RespuestaEvento("Eliminado con exito",HttpStatus.NO_CONTENT.value(), "delete"));
        }
        else
        {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(new RespuestaEvento("no eliminado",HttpStatus.NO_CONTENT.value(), "no-delete"));
        }
